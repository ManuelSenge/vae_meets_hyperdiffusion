...
class Encoder(nn.Module):
    def __init__(
        ...
        enc_chans_io, dec_chans_io = map(
            lambda t: list(zip(t[:-1], t[1:])), (self.enc_chans, self.dec_chans)
        )
        ...
        for (enc_in, enc_out), enc_kernel, d, att_flag, nhead in zip(
            enc_chans_io, enc_kernal_sizes, dims, self_attention, nheads
        ):
            self.enc_conv_layers.append(
                nn.Conv1d(
                    in_channels=enc_in,
            ...
            if att_flag:
                ...
                self.transformer_encoder.append(
                    nn.TransformerEncoder(encoder_layer, num_layers=num_att_layers)
                )
            else:
    ...
    def forward(self, x):
        ...
        for i in range(len(self.enc_conv_layers)):
            ...
            output = self.enc_conv_layers[i](
                output
            )  # torch.Size([BS, FILTER(8, 16, 32), height(499, 122, 30), width(499, 122, 3030)])
...
class Decoder(nn.Module):
    def __init__(
        ...
        enc_chans_io, dec_chans_io = map(
            lambda t: list(zip(t[:-1], t[1:])), (enc_chans, self.dec_chans)
        )
        ...
        self.transformer_encoder = []
        # add some conv layers
        for (dec_in, dec_out), dec_kernel, out_p, d, att_flag, nhead in zip(
            dec_chans_io, dec_kernal_sizes, output_padding, dims, self_attention, nheads
        ):
        ...
        self.linear = nn.Sequential(
            nn.Linear(latent_dim, self.linear_input_output_dim), act_fn
        )
    ...
    def forward(self, z):
        ...
        for i in range(len(self.dec_conv_layers)):
            ...
            output = self.dec_conv_layers[i](
                output
            )  # torch.Size([BS, FILTER(32, 16, 8), height(59, 122, 499), width(59, 122, 499)])
            output = self.activ_func(output)
            output = self.batch_normalization[i](output)
...
class VariationalAutoencoder(nn.Module):
    def __init__(
        ...
        enc_chans=[64, 32, 16, 1],
        enc_kernal_sizes=[8, 6, 3, 3],
        self_attention_encoder=[0, 0, 0, 0],
        self_attention_decoder=[0, 0, 0, 0],
        num_att_layers=4,
        variational=True,
    ):
        ...
        self.encoder = Encoder(
            ...
            enc_chans=enc_chans,
            enc_kernal_sizes=enc_kernal_sizes,
            device=device,
            self_attention=self_attention_encoder,
        ...
        self.decoder = Decoder(
            ...
            enc_chans=enc_chans,
            device=device,
            self_attention=self_attention_decoder,
            num_att_layers=num_att_layers,
        )
