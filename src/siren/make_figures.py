...
from moviepy.editor import (CompositeVideoClip, ImageSequenceClip, TextClip,
                            VideoClip, VideoFileClip, clips_array,
                            concatenate_videoclips, vfx)
from tensorboard.backend.event_processing import event_accumulator
from tqdm import tqdm


def extract_images_from_summary(
    events_path, tag_names_to_look_for, suffix="", img_outdir=None, colormap=None
):
    print("Extracting data from tensorboard summary...")
    event_acc = event_accumulator.EventAccumulator(
        events_path, size_guidance={"images": 0}
    )
    event_acc.Reload()
...
def save_video(video_clip, filepath):
    ...
    video_clip.resize(width=1080, height=height).write_videofile(
        filepath, fps=25, audio_codec="libfdk_aac", audio=False
    )


def make_video_grid_from_filepaths(
    num_rows,
    num_cols,
    video_list,
    trgt_name,
...
def animated_line_plot(
    x_axis, data, trgt_path, legend_loc="lower right", plot_type=None
):
    fig, ax = plt.subplots()
    fontdict = {"size": 16}
...
def make_video_from_tensorboard_summaries(
    summary_paths,
    trgt_path,
    image_extraction_dir,
    pred_tag_list,
    num_rows,
    num_cols,
    gt_tag_list=None,
    ...
    make_video_grid_from_filepaths(
        num_rows,
        num_cols,
...
def make_convergence_plot(
    gt_dir, img_dirs, trgt_path, animate=False, iters_info=None, plot_type=None
):
    """
    Args:
    ...
    if animate:
        ...
        fig.savefig(trgt_path, bbox="tight", bbox_inches="tight", pad_inches=0.0)
...
def image_convergence_video():
    ...
    make_video_from_tensorboard_summaries(
        summary_paths,
        trgt_path,
        image_extraction_dir=image_extraction_dir,
        pred_tag_list=pred_tag_list,
        num_rows=3,
        num_cols=len(summary_paths) + 1,
        gt_tag_list=gt_tag_list,
        overwrite=True,
    )
...
def image_convergence_plot():
    ...
    make_convergence_plot(
        None,
        img_dirs=summary_paths,
        animate=animated,
        trgt_path=filename,
        iters_info={"num_iters": 15001, "step": 5},
        plot_type="image",
    )
